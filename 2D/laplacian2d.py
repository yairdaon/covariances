...
def make_f( m, n ):
    ...
    f = f.real
    f = f - np.mean( f ) # take mean=0 because our covariance forces it
    return f / helper.norm( f )
...
if __name__ == "__main__":
    ...
    alpha = 0.25
    beta = 1 - alpha
    eigs = alpha + beta * lalapcian_eigenvalues( M, N, alpha ) 
    ...
    while True:
        ...
        eigenvalue = helper.dot( fourier_multiplier( eigenvector, eigs ), eigenvector )
        ...
        new = new / helper.norm( new )  
        
        # Take care of everything else
        res1 = helper.norm( new - eigenvector ) 
        res2 = helper.norm( new + eigenvector ) 
        if res1 > res2:
            eigenvector = -new 
            res = res2
        else:
